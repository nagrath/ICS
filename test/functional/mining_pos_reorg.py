...
class ReorgStakeTest(IQCashTestFramework):
    ...
    def check_money_supply(self, expected_ics, expected_zics):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected ICS and zICS supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_ics))
            for denom in node['zICSsupply']:
                assert_equal(node['zICSsupply'][denom], DecimalAmt(expected_zics[denom]))
    ...
    def run_test(self):
        ...
        # Check ICS and zICS supply at the beginning
        # ------------------------------------------
        # zICS supply: 2 coins for each denomination
        expected_zics_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # ICS supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zics_supply)
        ...
        # Verify that ICS and zICS supplies were properly updated after the spends and reorgs
        self.log.info("Check ICS and zICS supply...")
        ...
        expected_zics_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zics_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zics_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zics_supply)
