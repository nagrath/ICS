...
class ZerocoinSpendTest(IQCashTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zics_bal, ics_bal):
            zics_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zics_bal)
            ics_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], ics_bal)
            return zics_bal, ics_bal
        ...
        zics_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zics_balance, 6666)
        ...
        zics_balance, balance = check_balances(denom_2, zics_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zics_balance, balance = check_balances(denom_3, zics_balance, balance)
        ...
        _, _ = check_balances(denom_1, zics_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
