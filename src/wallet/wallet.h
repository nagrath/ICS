...
enum WalletFeature {
    ...
    FEATURE_PRE_IQCASH = 61000, // inherited version..
...
// Possible states for zICS send
enum ZerocoinSpendStatus {
    ZICS_SPEND_OKAY = 0,                            // No error
    ZICS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZICS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZICS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZICS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZICS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZICS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZICS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZICS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZICS_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZICS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZICS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZICS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZICS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZICS_SPENT_USED_ZICS = 14,                      // Coin has already been spend
    ZICS_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZICS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 10000 ICS output and keys which can be used for the Masternode
    ...
    void doZIcsRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZICS);
    ...
    bool CreateZICSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zICS wallet
    CzICSWallet* zwalletMain{nullptr};
    std::unique_ptr<CzICSTracker> zicsTracker{nullptr};
    void setZWallet(CzICSWallet* zwallet);
    CzICSWallet* getZWallet();
