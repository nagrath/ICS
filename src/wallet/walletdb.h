...
public:
    ...
    bool WriteZICSSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZICSSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZICSSeed_deprecated(uint256& seed);
    bool EraseZICSSeed();
    bool EraseZICSSeed_deprecated();

    bool WriteZICSCount(const uint32_t& nCount);
    bool ReadZICSCount(uint32_t& nCount);
