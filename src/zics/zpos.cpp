...
bool CLegacyZIcsStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZIcsStake from non zc-spend", __func__);
    ...
    *this = CLegacyZIcsStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zics stake origin", __func__);

    // All good
    return true;
}

CLegacyZIcsStake::CLegacyZIcsStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZIcsStake::GetIndexFrom()
{
...
CAmount CLegacyZIcsStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZIcsStake::GetUniqueness() const
{
...
bool CLegacyZIcsStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zICS stake block: height %d outside range", __func__, nHeight);
