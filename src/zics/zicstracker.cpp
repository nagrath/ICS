...
#include "zics/zicswallet.h"


CzICSTracker::CzICSTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzICSTracker::~CzICSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzICSTracker::Init()
{
...
bool CzICSTracker::Archive(CMintMeta& meta)
{
...
bool CzICSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzICSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzICSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzICSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzICSTracker::GetSerialHashes()
{
...
CAmount CzICSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzICSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzICSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzICSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzICSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzICSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzICSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzICSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzICSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzICSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzICSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzICSWallet* zICSWallet)
{
    bool iszICSWalletInitialized = (nullptr != zICSWallet);
    CMintMeta meta;
    ...
    if (!iszICSWalletInitialized)
        zICSWallet = new CzICSWallet(wallet);
    meta.isSeedCorrect = zICSWallet->CheckSeed(dMint);
    if (!iszICSWalletInitialized)
        delete zICSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzICSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzICSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzICSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzICSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzICSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzICSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzics from DB\n", __func__, listDeterministicDB.size());
    }
