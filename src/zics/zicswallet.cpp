...
CzICSWallet::CzICSWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zics seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZICSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZICSSeed_deprecated()) {
                    LogPrintf("%s: Updated zICS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zics seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zics wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzICSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZICSCount(nCountLastUsed);
    else if (!walletdb.ReadZICSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzICSWallet::Lock()
{
    seedMaster.SetNull();
}

void CzICSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzICSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZICS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzICSWallet::LoadMintPoolFromDB()
{
...
void CzICSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzICSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzICSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zicsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzICSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZICS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zicsTracker which also adds to database
    wallet->zicsTracker->Add(dMint, true);
...
void CzICSWallet::SeedToZICS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzICSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzICSWallet::GenerateDeterministicZICS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzICSWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzICSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzICSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
