...
#include "qt/iqcash/splash.h"
#include "qt/iqcash/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("iqcash-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in iqcash.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in iqcash.qrc)
...
/** Class encapsulating IQCASH Core startup and shutdown.
...
/** Main IQCASH application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (IQCASHGUI)
...
{
    window = new IQCASHGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &IQCASHGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &IQCASHGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(IQCASHGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(IQCASHGUI::DEFAULT_WALLET);
        ...
        // IQCASH: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &IQCASHGUI::handlePaymentRequest);
        connect(window, &IQCASHGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. IQCASH can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(iqcash_locale);
    Q_INIT_RESOURCE(iqcash);
    ...
    /// 6. Determine availability of data directory and parse iqcash.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("IQCASH Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("IQCASH Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // iqcash: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("IQCASH Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
