...
#include "qt/iqcash/topbar.h"
#include "qt/iqcash/forms/ui_topbar.h"
#include "qt/iqcash/lockunlock.h"
#include "qt/iqcash/qtutils.h"
#include "qt/iqcash/receivedialog.h"
#include "qt/iqcash/loadingdialog.h"
...
TopBar::TopBar(IQCASHGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopIcs}, "amount-small-topbar");
    setCssProperty({ui->labelAmountIcs}, "amount-topbar");
    setCssProperty({ui->labelPendingIcs, ui->labelImmatureIcs}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("IQCASH")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // ICS Total
    QString totalIcs = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);

    // ICS
    // Top
    ui->labelAmountTopIcs->setText(totalIcs);
    // Expanded
    ui->labelAmountIcs->setText(totalIcs);
    ui->labelPendingIcs->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureIcs->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
