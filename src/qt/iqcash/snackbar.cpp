...
#include "qt/iqcash/forms/ui_snackbar.h"
#include "qt/iqcash/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(IQCASHGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &IQCASHGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
