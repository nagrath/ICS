...
#include "qt/iqcash/dashboardwidget.h"
#include "qt/iqcash/forms/ui_dashboardwidget.h"
#include "qt/iqcash/sendconfirmdialog.h"
#include "qt/iqcash/txrow.h"
...
DashboardWidget::DashboardWidget(IQCASHGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareIcs, "square-chart-ics");
    setCssProperty(ui->labelSquarezIcs, "square-chart-zics");
    setCssProperty(ui->labelIcs, "text-chart-ics");
    setCssProperty(ui->labelZics, "text-chart-zics");
    ...
    setCssProperty(ui->labelAmountIcs, "text-stake-ics-disable");
    setCssProperty(ui->labelAmountZics, "text-stake-zics-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &IQCASHGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &IQCASHGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &IQCASHGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isIcs = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZICS;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair ICS, zICS
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ics = (pair.first != 0) ? pair.first / 100000000 : 0;
            zics = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalIcs += pair.first;
            chartData->totalZics += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesIcs.append(ics);
        chartData->valueszIcs.append(zics);

        int max = std::max(ics, zics);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesIcs);
    set1->append(chartData->valueszIcs);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalIcs > 0 || chartData->totalZics > 0) {
        setCssProperty(ui->labelAmountIcs, "text-stake-ics");
        setCssProperty(ui->labelAmountZics, "text-stake-zics");
    } else {
        setCssProperty(ui->labelAmountIcs, "text-stake-ics-disable");
        setCssProperty(ui->labelAmountZics, "text-stake-zics-disable");
    }
    forceUpdateStyle({ui->labelAmountIcs, ui->labelAmountZics});
    ui->labelAmountIcs->setText(GUIUtil::formatBalance(chartData->totalIcs, nDisplayUnit));
    ui->labelAmountZics->setText(GUIUtil::formatBalance(chartData->totalZics, nDisplayUnit, true));
