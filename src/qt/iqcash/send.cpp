...
#include "qt/iqcash/forms/ui_send.h"
#include "qt/iqcash/addnewcontactdialog.h"
#include "qt/iqcash/qtutils.h"
#include "qt/iqcash/sendchangeaddressdialog.h"
#include "qt/iqcash/optionbutton.h"
#include "qt/iqcash/sendconfirmdialog.h"
#include "qt/iqcash/myaddressrow.h"
#include "qt/iqcash/guitransactionsutils.h"
...
SendWidget::SendWidget(IQCASHGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-ics");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination icsAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(icsAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(icsAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
