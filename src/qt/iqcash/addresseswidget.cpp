...
#include "qt/iqcash/forms/ui_addresseswidget.h"
#include "qt/iqcash/addresslabelrow.h"
#include "qt/iqcash/addnewaddressdialog.h"
#include "qt/iqcash/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(IQCASHGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination icsAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(icsAdd, label.toUtf8().constData(),
