...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::ICS);
    ...
    if (settings.contains("nAnonymizeIQCashAmount") || reset)
        SoftSetArg("-anonymizeiqcashamount", settings.value("nAnonymizeIQCashAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in iqcash.cpp during shotdown to also remove the window positions
