...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/IQCASHParams"
    else
        PARAMS_DIR="$HOME/.iqcash-params"
    fi
fi
...
Failed to fetch the IQCASH zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
IQCASH - fetch-params.sh

This script will fetch the IQCASH zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common IQCASH zkSNARK parameters. Note that it is
