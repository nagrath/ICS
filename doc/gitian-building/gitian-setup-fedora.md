...
The build script we'll be using is contained in the IQCASH github repository ([contrib/gitian-build.py](https://github.com/IQCASH/IQCash/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the IQCASH repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/IQCASH/IQCash/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, iqcash-detached-sigs, and iqcash GitHub repos.
