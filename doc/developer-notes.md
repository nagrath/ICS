...
- [Development guidelines](#development-guidelines)
    - [General IQCASH Core](#general-iqcash-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
IQCASH Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play ICSs (tICS)" on the test network, if you
...
IQCASH Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/IQCASH/IQCash/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_iqcash
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_iqcash --log_level=test_suite
$ valgrind -v --leak-check=full src/iqcashd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_iqcash.coverage/index.html`.
```
...
- BitcoinMiner : Generates ICSs (if wallet is enabled).
...
However, in open source software such as IQCASH Core, where everyone uses
...
pay attention to for reviewers of IQCASH Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: IQCASH Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against IQCASH Core so that they can be integrated
...
file descriptors it will cause problems with IQCASH's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:IQCASH/IQCash.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `iqcash-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `iqcash-cli` and the GUI debug console use this table to determine how to
